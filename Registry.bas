...
Option Explicit
'*****************************************************************************************************
'**** Registry And INI-file handeling                                                             ****
'*****************************************************************************************************
'
' Written by Dave Scarmozzino, www.TheScarms.com
' Modified By Jens Vang Petersen
'
'
...
Private Declare Function GetPrivateProfileInt Lib "kernel32" _
        ...
        As String) As Long
Public Function fRecDeleteKey(ByVal sTopKey As String, ByVal sSubKey As String, ByVal sKeyName As String) As Long
    Dim Wsa As String, Wva As Variant, Wa As Long, Wb As Long
    Wb = fEnumKey(sTopKey, sSubKey + "\" + sKeyName, Wsa)
    If Wsa > " " Then
        Wva = Split(Wsa, Chr(0))
        For Wa = 0 To UBound(Wva)
            Wsa = Wva(Wa)
            If Wsa > " " Then Wb = fRecDeleteKey(sTopKey, sSubKey + "\" + sKeyName, Wsa)
            DoEvents
        Next Wa
    End If
    Wb = fDeleteKey(sTopKey, sSubKey, sKeyName)
    fRecDeleteKey = Wb
End Function
...
Public Function fReadValue(ByVal sTopKeyOrFile As String, _
    ...
    ByVal sValueType As String, ByVal vDefault As Variant) As Variant
    '
    ...
    If lTopKey = 1 Then
        ...
        If UCase$(sValueType) = "S" Then
            ...
            fReadValue = Left$(sValue, lResult)
        Else
            lDefault = 0
        ...
        Select Case UCase$(sValueType)
            Case "S"
                ...
                If lResult = ERROR_SUCCESS Then  'Remove null character.
                    fReadValue = Left$(sValue, lLenData - 1)
                Else
